...
def test_returns_matched_category(monkeypatch):
    monkeypatch.setattr(
        llm,
        "_get_client",
        lambda: make_fake_client({"matched_category": "coffee", "clarifying_question": ""}),
    )

    result = llm.parse_food_request("I want a flat white", ["coffee", "burger"])

    assert result == {"category": "coffee", "clarifying_question": None}
...
def test_returns_clarifying_question_when_no_match(monkeypatch):
    monkeypatch.setattr(
        ...
        lambda: make_fake_client(
            {"matched_category": "", "clarifying_question": "What kind of food are you in the mood for?"}
        ),
    )
...
def test_forces_the_extraction_tool_and_passes_categories(monkeypatch):
    fake_client = make_fake_client({"matched_category": "burger", "clarifying_question": ""})
